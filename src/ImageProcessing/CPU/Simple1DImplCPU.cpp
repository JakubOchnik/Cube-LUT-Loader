...
void Simple1DImplCPU::calculatePixel(const int x, const int y, const Table1D& lut, const WorkerData& data) {
	...
	const uchar b = data.image[triplet_idx + 0]; // b
	const uchar g = data.image[triplet_idx + 1]; // g
	const uchar r = data.image[triplet_idx + 2]; // r
	...
	data.newImage[triplet_idx + 0] = b + static_cast<uchar>((newB - b) * data.opacity);
	data.newImage[triplet_idx + 1] = g + static_cast<uchar>((newG - g) * data.opacity);
	data.newImage[triplet_idx + 2] = r + static_cast<uchar>((newR - r) * data.opacity);
}
